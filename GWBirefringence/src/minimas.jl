...
"""
    find_minimum(floss::Function, options, algorithm=NelderMead();
                 Nmax::Int64=100, atol::Float64=1e-12)

Find minimum of a function `floss(p)`, where `p` are the sampled initial
conditions.
"""
function find_minimum(floss::Function, options, algorithm=NelderMead();
                      Nmax::Int64=100, atol::Float64=1e-12)
    for i in 1:Nmax
        p0 = GWBirefringence.uniform_sample_sphere()
        opt = optimize(floss, p0, algorithm, options)
...
"""
    cluster_minima(X::T,
                   tol::Float64=1e-10) where T<:Union{Matrix{GWFloat},
                                                      Vector{Vector{GWFloat}}}
...
function cluster_minima(
        ...
        tol::Float64=1e-10) where T<:Union{Matrix{GWFloat},
                                           Vector{Vector{GWFloat}}}
    # Optionally convert to a matrix
...
"""
    search_unique_minima(fmin::Function, N::Int64;
                         Nmax::Int64=50, atol::Float64=1e-12)
